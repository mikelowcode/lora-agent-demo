...
**Status: fully built, 2026-09-06.** All four gates now resolve to a real "lexical-fallback" tier (`planner._LEXICAL_FALLBACK_THRESHOLDS`, calibrated via `diagnostics/calibrate_lexical_fallback_thresholds.py`/`threshold_calibration.calibrate_lexical_thresholds()`) whenever a gate has no working embedding-based threshold — both halves of the motivation below: a real, *named*, non-tuned embedding model with no validated/persisted-calibration entry for a gate, AND true zero-config keyword-only (`embedding_model_name is None`, `embed_fn is None`). The keyword-only half shipped in a same-day follow-up after an initial pass deliberately deferred it: a first attempt at wiring it in broadly (via `resolve_gate_tiers()` itself) broke ~28 pre-existing tests that construct a keyword-only `Planner` as an isolated-unit-test shortcut for unrelated P3 sub-features (github/hacker-news/url-fetch guards, the explicit-date signal, etc.). The fix landed as a narrower, correct one: `Planner._lexical_fallback_active(name)` checks `self._embed_fn is None` directly rather than relying on the tier label (`resolve_gate_tiers(None, …)` still reports `"tuned"` for labeling purposes only, since that label is inert without embed_fn anyway), and the ~7 genuinely affected tests were updated forward — most needed only a mocked-cosine-isolation helper (`_planner_with_mocked_semantic()`) to also neutralize the new lexical path via `_lexical_fallback_active`, and two needed their `tools_to_call` assertions relaxed from exact-list equality to membership checks, since a keyword-only session can now legitimately get an additional `web_search` signal alongside its guarded tool. See `docs/architecture/16-runtime-backend-layer.md` §16.17 for the full detail.
