...
**Status: built 2026-09-06, with one deliberate scope reduction from the original design below.** All four gates now resolve to a real "lexical-fallback" tier (`planner._LEXICAL_FALLBACK_THRESHOLDS`, calibrated via `diagnostics/calibrate_lexical_fallback_thresholds.py`/`threshold_calibration.calibrate_lexical_thresholds()`) whenever a real, *named*, non-tuned embedding model has no validated or persisted-calibration entry for that gate — exactly the "a model calibrates as degenerate for a gate" half of the motivation below. The "true zero-config keyword-only" half (`embedding_model_name is None`, `embed_fn is None`) is **not** covered by this pass: `Planner._lexical_fallback_active()` deliberately excludes it, because a very large pre-existing test surface (P3's github/hacker-news/url-fetch guard tests, the explicit-date signal tests, and others) constructs a keyword-only `Planner` specifically to test unrelated literal-keyword logic in isolation, with zero additional signal expected — extending lexical fallback there produced real routing changes across dozens of unrelated tests when first attempted. Tracked as an explicit, real follow-up, not silently dropped — see `Planner._lexical_fallback_active()`'s docstring and `docs/architecture/16-runtime-backend-layer.md` §16.17's Open items.
